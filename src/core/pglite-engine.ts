  ...
  private async applyForwardReferenceBootstrap(): Promise<void> {
    ...
    const { rows } = await this.db.query(`
      SELECT
        ...
        EXISTS (SELECT 1 FROM information_schema.columns
                WHERE table_schema='public' AND table_name='content_chunks' AND column_name='doc_comment') AS doc_comment_exists,
        EXISTS (SELECT 1 FROM information_schema.columns
                WHERE table_schema='public' AND table_name='content_chunks' AND column_name='symbol_name_qualified') AS symbol_name_qualified_exists
    `);
    ...
    if (needsChunksBootstrap) {
      ...
      // v27/v28 (cathedral_ii) adds the chunk-grain FTS column and qualified-
      // symbol metadata. The bootstrap only adds the columns the schema blob's
      // indexes (idx_chunks_symbol_name, idx_chunks_language,
